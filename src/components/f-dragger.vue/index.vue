...
export default {
    ...
    props: {
        ...
        rangeMode: {
            ...
            default: 'inside',
            validator: (value) =>
                ['inside', 'center', 'outside'].includes(value),
        },
    ...
    methods: {
        getRange(transferEl) {
            ...
            if (typeof this.range === 'object') range = this.range;
                ...
                range = Object.assign(
                    { left: 0, top: 0 },
                    getSize(offsetParent, this.rangeMode),
                );
            ...
            if (params.range) {
                if (this.rangeMode === 'inside') {
                    next.left = Math.min(
                        ...
                        params.range.right - manager.transferEl.offsetWidth,
                    );
                    next.top = Math.min(
                        Math.max(params.range.top, next.top),
                        params.range.bottom - manager.transferEl.offsetHeight,
                    );
                    ...
                    next.left = Math.min(
                        Math.max(
                            params.range.left - manager.transferEl.offsetWidth,
                            next.left,
                        ),
                        params.range.right,
                    );
                    next.top = Math.min(
                        Math.max(
                            params.range.top - manager.transferEl.offsetHeight,
                            next.top,
                        ),
                        params.range.bottom,
                    );
