...
export default {
    ...
    render() {
        const referenceVnode = this.$slots.reference ? this.$slots.reference[0] : '';
    ...
    methods: {
        ...
        updatePopoverVM() {
            if (this.isScopedSlots) {
                const props = Object.assign({}, this._data, this._props);
        ...
        hidePopoverDomEvent(e) {
            const _t = e.target, _r = this.reference, _p = this.popover;
            if (!_r || !_p || _p.contains(_t) || _r.contains(_t))
                return false;
            this.show = false;
        },
        showPopoverDelay() {
